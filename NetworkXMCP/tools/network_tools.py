...
def fix_graphml_structure(graphml_content):
    ...
    try:
        ...
        if "<data " in graphml_content and "</data>" not in graphml_content:
            ...
            graphml_content = re.sub(
                r'<data key="([^"]+)"></data>', r'<data key="\1"/>', graphml_content)
    except Exception as e:
...
def convert_to_standard_graphml(graphml_content):
    ...
    さらに、スプリングレイアウトを自動的に適用して位置情報を設定する
    ...
    try:
        ...
        # デバッグ情報を追加
        logger.debug(
            f"GraphML content before fixing: {graphml_content[:500]}...")
        ...
        try:
            ...
            if "XML" in error_details:
                ...
                try:
                    ...
                    G = nx.read_graphml(content_io)
                    logger.debug(
                        f"Successfully parsed GraphML after XML fixes with {G.number_of_nodes()} nodes and {G.number_of_edges()} edges")
                except Exception as second_parse_error:
                    logger.error(
                        f"Error parsing GraphML after XML fixes: {second_parse_error}")
                    return {
                        "success": False,
        ...
        try:
            if G.number_of_nodes() > 0:
                ...
                try:
                    ...
                    logger.warning(
                        "Eigenvector centrality did not converge, setting to 0.")
                ...
                nx.set_node_attributes(
                    G, closeness_centrality, "closeness_centrality")
                nx.set_node_attributes(
                    G, betweenness_centrality, "betweenness_centrality")
                nx.set_node_attributes(
                    G, eigenvector_centrality, "eigenvector_centrality")
                logger.debug(
                    "Successfully calculated and set centrality attributes.")
            else:
                logger.debug(
                    "Graph has no nodes, skipping centrality calculation.")
        ...
        # スプリングレイアウトを自動適用
        logger.debug("Automatically applying spring layout...")
        try:
            if G.number_of_nodes() > 0:
                # スプリングレイアウトを計算
                ...
                logger.debug(
                    f"Successfully applied spring layout to {len(positions)} nodes")
            else:
                logger.debug(
                    "Graph has no nodes, skipping spring layout calculation.")
        except Exception as layout_error:
            logger.error(f"Error applying spring layout: {layout_error}")
            # レイアウト計算でエラーが発生しても、処理は続行する
            # デフォルトの位置を設定
            for i, node in enumerate(G.nodes()):
                import random
                G.nodes[node]['x'] = str(random.uniform(-1.0, 1.0))
                G.nodes[node]['y'] = str(random.uniform(-1.0, 1.0))
        ...
        try:
            ...
            try:
                ...
                for node, attrs in G.nodes(data=True):
                    ...
                    minimal_graphml.append(
                        f'      <data key="d0">{escape(str(attrs.get("name", f"Node {node}")))}</data>')
                    minimal_graphml.append(
                        f'      <data key="d1">{escape(str(attrs.get("size", "5.0")))}</data>')
                    minimal_graphml.append(
                        f'      <data key="d2">{escape(str(attrs.get("color", "#1d4ed8")))}</data>')
                    minimal_graphml.append(
                        f'      <data key="d3">{escape(str(attrs.get("description", f"Node {node}")))}</data>')
                    minimal_graphml.append(
                        f'      <data key="d4">{escape(str(attrs.get("x", "0.0")))}</data>')
                    minimal_graphml.append(
                        f'      <data key="d5">{escape(str(attrs.get("y", "0.0")))}</data>')
                ...
                for u, v, attrs in G.edges(data=True):
                    ...
                    v_str = escape(str(v), {"'": "&apos;", '"': "&quot;"})
                    minimal_graphml.append(
                        f'    <edge source="{u_str}" target="{v_str}"/>')
                ...
                try:
                    ...
                    if len(parts) == 2:
                        ...
                        if len(graphml_tag_parts) == 2:
                            key_str = ">\n  " + \
                                "\n  ".join(key_elements) + "\n  "
                            standardized_graphml = parts[0] + "<graphml " + \
                                graphml_tag_parts[0] + \
                                key_str + graphml_tag_parts[1]
                        else:
                            # 通常のケース
                            key_str = "\n  " + \
                                "\n  ".join(key_elements) + "\n  "
                            standardized_graphml = parts[0] + "<graphml " + \
                                parts[1].replace(
                                    "<graph ", key_str + "<graph ", 1)
                    else:
                        logger.warning(
                            "Could not find <graphml> tag for inserting <key> elements")
                        ...
                        if standardized_graphml.strip().startswith("<?xml"):
                            ...
                            if len(xml_parts) == 2:
                                key_str = "?>\n<graphml>\n  " + \
                                    "\n  ".join(key_elements) + "\n  "
                                standardized_graphml = xml_parts[0] + \
                                    key_str + xml_parts[1]
                        else:
            ...
            logger.debug(
                f"Final standardized GraphML (first 500 chars): {standardized_graphml[:500]}...")
...
def export_network_as_graphml(G, positions=None, visual_properties=None):
    ...
    try:
        ...
        if visual_properties:
            ...
            export_G.graph['node_default_size'] = str(
                visual_properties.get('node_size', 5))
            export_G.graph['node_default_color'] = visual_properties.get(
                'node_color', '#1d4ed8')
            export_G.graph['edge_default_width'] = str(
                visual_properties.get('edge_width', 1))
...
def apply_layout_to_graphml(graphml_content, layout_type="spring", layout_params=None):
    ...
    try:
        ...
        # スプリングレイアウトの場合はより良いデフォルトパラメータを設定
        if layout_type == "spring":
            layout_params.setdefault("k", 1.0)
            layout_params.setdefault("iterations", 50)
            layout_params.setdefault("seed", 42)

        # レイアウトを計算
        logger.debug(
            f"Calculating {layout_type} layout with params: {layout_params}")
        try:
        ...
        logger.debug(
            f"Successfully applied {layout_type} layout to {len(positions_dict)} nodes")

...
提供されているシェルスクリプトを使用して、簡単にセットアップと起動ができます。
...
1.  **初回セットアップ**
    ...
    - 前提条件のチェック（Python 3.12+、Node.js、PostgreSQL）
    - `.env`ファイルの作成と設定
    - PostgreSQLデータベースとテーブルの作成
    - Python仮想環境の作成と依存関係のインストール
    - Node.js依存関係のインストール

2.  **環境変数の設定**

    `.env`ファイルを編集して、API_KEYを設定してください：
...
5.  **ログの確認**
    ...
    各サービスのログは`logs/`ディレクトリに保存されます：

    ```bash
    # リアルタイムでログを確認
    tail -f logs/api.log
    tail -f logs/networkx.log

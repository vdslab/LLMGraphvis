...
- **技術選定**:
  - **可視化ライブラリ**: **D3.js** を推奨する。描画自体に複雑な計算を必要とせず、高い柔軟性と表現力を提供するため。NetworkXで処理されたグラフデータは、標準的なJSON形式（node-linkデータなど）に変換することでD3.jsで容易に描画可能である。ただし、D3.jsはWebGLベースのSigma.jsと比較して、数万ノード規模の大規模グラフの描画にはパフォーマンス面で不向きであるという点に留意する必要がある。もし将来的に大規模グラフの描画が必要になった場合、パフォーマンス上のボトルネックとなる可能性があり、その際はSigma.jsのようなWebGLベースのライブラリへの移行、またはD3.jsとWebGLレンダリングを組み合わせるなどの検討が必要になる。
- **状態同期とUI**:
...
*   **永続化の仕組み**: ユーザーがグラフデータをアップロードすると、バックエンドがデータを解析し、Neo4jにノードとリレーションシップとして投入する。これにより、グラフ構造のネイティブな保存と、Cypherによる効率的なトラバーサルおよびパターンマッチングクエリが可能となる。NetworkXはメモリ内計算エンジンとして利用され、Neo4jから取得したデータでグラフオブジェクトを構築する。

#### Redis (インメモリデータストア / メッセージブローカー)
*   **役割**: 高速キャッシュ層およびメッセージブローカーとして機能する。
*   **保存されるデータ**:
    *   **計算済みメトリクスとレイアウト座標**: NetworkXで計算された中心性スコアやレイアウト座標などの派生データ。これらは`project:{project_id}:graph:{graph_id}:metric:betweenness`のようなキーで保存され、再計算コストを削減し、高速な取得を可能にする。
    ...
    *   **JWT失効リスト**: JWTアクセストークンのステートレス性に対応するため、失効したリフレッシュトークンのIDをRedisのブラックリストで管理する。
*   **永続化の仕組み**: 計算結果はCeleryワーカーによってRedisに直接書き込まれる。WebSocketの状態管理ではPub/Sub機能が利用され、JWT失効リストはTTL（Time to Live）を設定して管理される。サブミリ秒のレイテンシでのデータ読み書きが強み。

#### PostgreSQL (リレーショナルデータベース)
*   **役割**: アプリケーションのプライマリデータベースとして、構造化されたリレーショナルデータを管理する。
*   **保存されるデータ**: ユーザーアカウント、プロジェクトメタデータ、認証情報、保存された可視化設定、チャット履歴。
*   **永続化の仕組み**: ユーザー登録、プロジェクト作成、設定変更、チャット履歴の保存などの際に、ACID準拠のトランザクションを通じてデータが永続化される。強力なデータ整合性とトランザクションの信頼性が強み。

...
async def run_verification():
    ...
    chunk1.candidates[0].content.parts = [types.Part(text="[Phase 1] I will check the metadata.")]
    ...
    func_call = types.FunctionCall(name="read_resource", args={"uri": "network://1/metadata"})
    ...
    chunk3.candidates[0].content.parts = [types.Part(text="[Phase 3] I have checked the metadata.")]
    
    async def second_stream_iterator():
        yield chunk3

    # Mock Dependencies
    with patch("app.services.llm.mcp_client.execute_tool", new_callable=AsyncMock) as mock_tool:
        mock_tool.return_value = {"name": "Test Network", "node_count": 10}
        
        # Mock client for next iteration
        with patch("app.services.llm.engine.client.aio.models.generate_content_stream", return_value=second_stream_iterator()) as mock_client_call:
             
             # Mock local_tools
             with patch("app.services.llm.local_tools.execute_local_tool", new_callable=AsyncMock):
    ...
    has_text_1 = any(e['event'] == 'message_chunk' and "Phase 1" in e['data'] for e in events)
    has_tool_start = any(e['event'] == 'tool_execution' and "started" in e['data'] for e in events)
    has_tool_end = any(e['event'] == 'tool_execution' and "completed" in e['data'] for e in events)
    has_text_2 = any(e['event'] == 'message_chunk' and "Phase 3" in e['data'] for e in events)

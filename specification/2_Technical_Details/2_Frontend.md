...
質問はモデル生成HTMLではなく、型と制約を持つ入力要求として表示する。
選択肢やスライダーの初期値は回答を意味せず、送信操作があって初めて回答になる。
質問をツール実行結果と結びつけることで、通常のメッセージ履歴と同じ順序で
再表示できる。SSEに接続していたことを復元の前提にしない。

回答済みかどうかはサーバーの永続状態を確認する。別のネットワークへ移った場合や
新しい質問に置き換わった場合は古いフォームを使えない。入力の型・範囲・選択肢は
サーバーでも検証し、再送が分析の二重実行につながらないようにする。
自由文は引き続き通常のチャット入力から送れ、提案された選択肢に意図が収まらない
場合にも会話を続けられる。背景はADR [0004](../4_Decisions/0004-conversation-inputs.md)。

候補の提示は目的を制限するものではないため、選択式質問には既定で自由入力の
逃げ道を付ける。「その他」を選んだ時だけ入力欄を表示し、候補に戻した場合は
使わなくなった自由入力を送信しない。回答済み表示でも自由入力内容を読める形で
保持し、候補の選択と同様に再送信を防ぐ。

分析ツールの失敗はツール欄に理由を表示し、コンソールには警告として記録する。
未作成の派生属性を読む呼び出しは失敗ではなく `deferred` として中立色で表示する。
SSE の破損や画面処理の例外は引き続きコンソールエラーとし、ブラウザ検証で
区別できるようにする。

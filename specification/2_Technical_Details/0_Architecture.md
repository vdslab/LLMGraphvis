...
graph TD
    ...
    llmServices["LLM Services<br/>交換可能なプロバイダ<br/>(クラウド / ローカル)"]
...
| **LLM Services**    | 指示解釈、ツールプランニング、応答生成に用いる大規模言語モデル。特定ベンダに固定せず、プロバイダ抽象を介して差し替え可能とする（0.6 参照）。 |
...
UI の外観は下図の通り。左に会話、右に可視化キャンバスを置き、会話が唯一の操作手段であることを示す。

![Agent UI コンセプト](../images/agent_ui_concept.png)
...
- **サーバーサイド (Database)**:
  ...
  - **永続化されるデータ**:
    ...
    - 各ユーザーの会話履歴
    - メッセージ（ユーザーの発言、LLMの応答）とツール実行の記録
    - ネットワークの構造（ノード・エッジ）と属性データ。元データ由来のものと、計算によって追加されたレイアウト座標や中心性指標の双方を含む
  - **設計判断**: アップロードされた GraphML は原文のまま保存せず、インポート時にノード・エッジ・属性へ分解して格納する。分析対象は個々の属性値であり、原文を保持しても再パースが必要になるだけで利点がないため。したがって「元の GraphML ファイル」を復元する経路は存在せず、エクスポートは常に現在の DB 状態からの再生成となる。
...
- **状態の保存**: 部分的な更新（例：レイアウトのみ変更）を行う際、既存の視覚設定（色など）は可能な限り維持する。 `visualization_get_state` を用いて現在ユーザーが見ている状態を把握してから変更を加える。

### 0.5.4. Action First & Verification
- **即時実行**: 計画を語るだけでなく、可能な限りそのターンでツールを実行する。
- **検証**: 属性に基づいて操作を行う際は、`network_list_node_attributes` 等を用いてデータの存在確認を行うことが推奨される。

> Agent Policy の実装は `backend/app/services/llm/prompts.py`（常時ポリシー）と `backend/app/services/llm/hooks/builtin/`（強制）に分かれる。プロンプトの指示は無視されうるが、フックによる拒否は無視できない。この使い分けの根拠は [1_Backend.md](./1_Backend.md) を参照。

## 0.6. LLM プロバイダの抽象化

### 0.6.1. 決定

エージェントループは特定の LLM ベンダの SDK に直接依存しない。`backend/app/services/llm/providers/` に置かれた抽象基底クラス `LLMProvider` を介してのみモデルと会話し、具体的なアダプタ（クラウド商用モデル群およびローカル実行モデル）はその実装として差し替える。

利用するプロバイダとモデルは環境変数で既定を与えつつ、**チャットごとに永続化される**。会話の途中でモデルを切り替えても、その会話は最後に選ばれたモデルで再開する。

### 0.6.2. 根拠

- **研究上の要請**: 本システムの評価対象はエージェントの振る舞いであり、単一モデルに固定すると「観測した挙動がモデル固有か設計由来か」を切り分けられない。同一のツール群・同一のプロンプトに対して複数モデルを差し替えられることが、評価の前提条件になる。
- **再現性とコスト**: ローカル実行モデルを同じ経路で扱えることで、外部 API の課金と可用性に依存せずに実験を反復できる。
- **契約の一本化**: アダプタは、ストリーム片・ツール呼び出し・思考の 3 つを共通の型に正規化して返す。この正規化があるため、エンジン側にベンダ分岐が入らない。

### 0.6.3. 帰結

- 新しいプロバイダの追加は、抽象基底クラスの実装を 1 つ増やすことに閉じる。エンジン・フック・スキルは変更しない。
- **思考の扱いだけはプロバイダごとに異なる。** ネイティブの推論ストリームを持つモデルと、持たないためにプロンプトで `<thought>` タグを書かせるモデルがある。両方を同時に要求すると出力が二重化するため、エンジンはプロバイダの申告に応じてシステムプロンプトの変種を切り替える。この分岐はプロバイダ抽象が吸収しきれない唯一の差であり、意図的に露出させている（`LLMProvider.supports_native_thinking`）。
- モデルとプロバイダが会話に紐づく以上、会話の再生には当時のモデル情報が必要になる。データモデルはこれを保持する（[4_Database.md](./4_Database.md)）。

...
def create_test_network(db):
    ...
    # Edge
    e1 = models.Edge(
        id=1, network_id=999, edge_id="e1", source_node_id=1, target_node_id=2
    )
    db.add(e1)
    ...
    lay_x = models.NodeAttribute(
        id=100, network_id=999, attribute_name="forceatlas2_x", data_type="float"
    )
    lay_y = models.NodeAttribute(
        id=101, network_id=999, attribute_name="forceatlas2_y", data_type="float"
    )
    db.add(lay_x)
    db.add(lay_y)
...
def test_missing_attribute_raises_error(db):
    ...
    with pytest.raises(ValueError) as excinfo:
        visualizer.generate_visualization_data(
            network_id, db, node_size_config=node_size_config
        )

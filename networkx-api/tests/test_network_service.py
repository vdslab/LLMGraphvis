...
def test_update_network_metadata(db):
    ...
    # Execute
    msg = network_service.update_network_metadata(db, network.id, name="NewName", description="NewDesc")
...
def test_get_network_structure(db):
    ...
    e1 = models.Edge(network_id=network.id, edge_id="e1", source_node_id=n1.id, target_node_id=n2.id)
    db.add(e1)
    db.commit()

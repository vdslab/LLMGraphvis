...
def setup_dummy_network(db: Session) -> int:
    ...
    nodes = []
    for i in range(5):
        nodes.append(models.Node(network_id=network.id, node_id=str(i), label=f"Node {i}"))
    db.add_all(nodes)
    db.commit()
    ...
    edges = []
    for i in range(4):
        node_u = db.query(models.Node).filter(models.Node.network_id == network.id, models.Node.node_id == str(i)).first()
        node_v = db.query(models.Node).filter(models.Node.network_id == network.id, models.Node.node_id == str(i+1)).first()
        edges.append(models.Edge(network_id=network.id, edge_id=f"e{i}", source_node_id=node_u.id, target_node_id=node_v.id))
    db.add_all(edges)
    
    # Add Attributes
    attr = models.NodeAttribute(network_id=network.id, attribute_name="age", data_type="float")
    db.add(attr)
    db.commit()
    db.refresh(attr)
    ...
    vals = []
    for i in range(5):
        node = db.query(models.Node).filter(models.Node.network_id == network.id, models.Node.node_id == str(i)).first()
        nav = models.NodeAttributeValue(node_id=node.id, attribute_id=attr.id)
        db.add(nav)
        db.commit()
        db.refresh(nav)
        
        val = models.NodeFloatAttributeValue(node_attribute_value_id=nav.id, float_value=float((i+1)*10))
        db.add(val)
    db.commit()
...
def test_filter_subgraph_description(db_session):
    ...
    cond = AttributeCondition(
        attribute_name="age",
        ranges=[Range(min=30.0)]
    )
    
    result = filter.create_subgraph_by_filter(net_id, [cond], "Filtered", db_session)
    sub_id = result["new_network_id"]
    
    subgraph_net = db_session.query(models.Network).filter(models.Network.id == sub_id).first()

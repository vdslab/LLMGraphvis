...
def test_subgraph_fresh_view(db: Session):
    ...
    for n in nodes:
        ...
        db.add(
            models.NodeAttributeValue(
                node_id=n.id,
                attribute_id=attr_x.id,
                float_value=models.NodeFloatAttributeValue(float_value=10.0),
            )
        )
        db.add(
            models.NodeAttributeValue(
                node_id=n.id,
                attribute_id=attr_y.id,
                float_value=models.NodeFloatAttributeValue(float_value=10.0),
            )
        )
        # Metric values
        db.add(
            models.NodeAttributeValue(
                node_id=n.id,
                attribute_id=attr_deg.id,
                float_value=models.NodeFloatAttributeValue(float_value=2.0),
            )
        )
        ...
        db.add(
            models.NodeAttributeValue(
                ...
                float_value=models.NodeFloatAttributeValue(float_value=99.0),
            )
        )
    db.commit()
    ...
    result = subgraph.create_subgraph_from_nodes(
        network.id, node_ids, db, suffix="Fresh", preserve_layout=False
    )
    ...
    new_attrs = (
        ...
        .filter(models.NodeAttribute.network_id == new_net_id)
        .all()
    )
...
def test_subgraph_cutout_view(db: Session):
    ...
    source_pos = {"0": (1.0, 1.0), "1": (2.0, 2.0), "2": (3.0, 3.0)}
    ...
    for n in nodes:
        ...
        db.add(
            models.NodeAttributeValue(
                node_id=n.id,
                attribute_id=attr_x.id,
                float_value=models.NodeFloatAttributeValue(float_value=px),
            )
        )
        db.add(
            models.NodeAttributeValue(
                node_id=n.id,
                attribute_id=attr_y.id,
                float_value=models.NodeFloatAttributeValue(float_value=py),
            )
        )
        db.add(
            models.NodeAttributeValue(
                node_id=n.id,
                attribute_id=attr_deg.id,
                float_value=models.NodeFloatAttributeValue(float_value=5.0),
            )
        )
    db.commit()
    ...
    result = subgraph.create_subgraph_from_nodes(
        network.id, ["0", "1"], db, suffix="Cutout", preserve_layout=True
    )
    new_net_id = result["new_network_id"]

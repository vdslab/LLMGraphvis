...
<graphml xmlns="http://graphml.graphdrawing.org/xmlns"  
  ...
  <graph id="G" edgedefault="undirected">
    ...
    # Check for forceatlas2 capability first
    if hasattr(nx, 'forceatlas2_layout'):
        print("✅ NetworkX has forceatlas2_layout attribute.")
    else:
        print("❌ NetworkX MISSING forceatlas2_layout attribute.")
        return
    ...
    # Verify Spring Layout (ensure no regression)
    layout_name = "spring"
    print(f"Testing layout: {layout_name} (Regression Check)...")
    try:
        layout.calculate_layout(network_id, layout_name, db)
        ...
        if attr_x:
            vals_x = db.query(models.NodeFloatAttributeValue).join(models.NodeAttributeValue).filter(models.NodeAttributeValue.attribute_id == attr_x.id).count()
            if vals_x > 0:
                print(f"✅ SUCCESS: {layout_name} works.")
            else:
                 print(f"❌ FAILURE: {layout_name} has no values.")
        else:
             print(f"❌ FAILURE: {layout_name} attribute missing.")

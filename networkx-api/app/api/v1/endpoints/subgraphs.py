...
def get_subgraphs(network_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return [
            {"id": s.id, "name": s.name, "created_at": str(s.created_at)}
            for s in subgraphs
        ]
...
def create_ego_network(
    network_id: int, request: EgoNetworkRequest, db: Session = Depends(get_db)
):
    try:
        return subgraph.create_ego_network(
            network_id, request.center_node_id, request.radius, db
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def create_subgraph_from_nodes(
    network_id: int, request: NodesSubgraphRequest, db: Session = Depends(get_db)
):
    try:
...
def create_path_subgraph(
    network_id: int, request: PathSubgraphRequest, db: Session = Depends(get_db)
):
    try:
        return subgraph.create_path_subgraph(
            network_id, request.source_node_id, request.target_node_id, db
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def create_k_core_subgraph(
    network_id: int, request: KCoreRequest, db: Session = Depends(get_db)
):
    try:
...
def create_component_containing_node(
    network_id: int,
    request: ComponentContainingNodeRequest,
    db: Session = Depends(get_db),
):
    try:
        return subgraph.create_component_containing_node(
            network_id, request.node_id, db
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

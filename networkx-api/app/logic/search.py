...
def search_nodes(
    ...
    """
    logger.info(
        f"Searching nodes in network {network_id} (query='{query}', attr='{attribute_name}', limit={limit})"
    )
...
def _search_by_attribute(
    db: Session, network_id: int, query: str, attribute_name: str, limit: int
) -> List[NodeSearchResult]:
    """
    ...
    query_results = (
        db.query(models.Node, models.NodeTextAttributeValue.text_value)
        .join(
        ...
        .join(
            ...
            models.NodeAttributeValue.attribute_id == models.NodeAttribute.id,
        )
        .join(
...
def _search_default(
    db: Session, network_id: int, query: str, limit: int
) -> List[NodeSearchResult]:
    """
    ...
    for node in query_results:
        ...
        match_val = (
            node.label
            if node.label and query.lower() in node.label.lower()
            else node.node_id
        )

        results.append(
            {"id": node.node_id, "label": node.label, "match": match_val, "score": 1.0}
        )
    return results


def get_node_details(
    network_id: int, node_id: str, db: Session
    ...
    float_attrs = (
        db.query(
            models.NodeAttribute.attribute_name,
            models.NodeFloatAttributeValue.float_value,
        )
        .join(
            models.NodeAttributeValue,
            models.NodeAttribute.id == models.NodeAttributeValue.attribute_id,
        )
        .join(
        ...
        .filter(models.NodeAttributeValue.node_id == node.id)
        .all()
    )

...
class AttributeCopier:
    ...
    def copy_attributes(
        ...
        logger.info(
            f"Copying attributes from {source_network_id} to {new_network_id} (excluding: {excluded_attributes})"
        )

        # Copy Node Attribute Definitions
        node_attr_id_map = self._copy_attribute_definitions(
            source_network_id, new_network_id, models.NodeAttribute, excluded_attributes
        )

        # Copy Edge Attribute Definitions
        edge_attr_id_map = self._copy_attribute_definitions(
            source_network_id, new_network_id, models.EdgeAttribute, excluded_attributes
        )
    ...
    def _copy_attribute_definitions(
        self,
        source_network_id: int,
        new_network_id: int,
        model_class,
        ...
        source_attrs = (
            ...
            .filter(model_class.network_id == source_network_id)
            .all()
        )
        attr_id_map = {}
        ...
        if new_attrs_data:
            ...
            new_attrs = (
                self.db.query(model_class)
                .filter(model_class.network_id == new_network_id)
                .all()
            )
    ...
    def _copy_values(
        ...
        if not old_ids or not old_attr_ids:
            return
        ...
        for i in range(0, total, chunk_size):
            ...
            for val in old_vals:
                ...
                if new_pk and new_attr_id:
                    new_vals_data.append(
                        {parent_col: new_pk, "attribute_id": new_attr_id}
                    )
            ...
            if new_vals_data:
                ...
                val_map = {
                    (getattr(v, parent_col), v.attribute_id): v.id
                    for v in inserted_vals
                }
                ...
                if old_val_ids:
                    ...
                    for j in range(0, len(old_val_ids), sub_chunk_size):
                        ...
                        old_floats.extend(
                            self.db.query(model_float)
                            .filter(getattr(model_float, val_parent_col).in_(sub_ids))
                            .all()
                        )
                        old_texts.extend(
                            self.db.query(model_text)
                            .filter(getattr(model_text, val_parent_col).in_(sub_ids))
                            .all()
                        )

                    float_map = {
                        getattr(f, val_parent_col): f.float_value for f in old_floats
                    }
                    text_map = {
                        getattr(t, val_parent_col): t.text_value for t in old_texts
                    }
                        ...
                        if new_pk and new_attr_id:
                            ...
                            if new_val_id:
                                if val.id in float_map:
                                    new_float_data.append(
                                        {
                                            val_parent_col: new_val_id,
                                            "float_value": float_map[val.id],
                                        }
                                    )

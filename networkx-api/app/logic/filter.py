...
def _get_nodes_matching_condition(network_id: int, condition: AttributeCondition, db: Session) -> Set[str]:
    ...
    if not attr_record:
        logger.warning(f"Attribute '{attr_name}' not found in network {network_id}. Treating as no match.")
        return set()
    ...
    query = db.query(models.Node.node_id).join(
        models.NodeAttributeValue, models.Node.id == models.NodeAttributeValue.node_id
    ).filter(
        models.NodeAttributeValue.attribute_id == attr_id
    )
    ...
    if condition.categories:
        for cat in condition.categories:
            if isinstance(cat, str):
                filters.append(
                    models.NodeAttributeValue.text_value.has(models.NodeTextAttributeValue.text_value == cat)
                )
            else:
                # Numeric categories (treat as float exact match)
                filters.append(
                    models.NodeAttributeValue.float_value.has(models.NodeFloatAttributeValue.float_value == float(cat))
                )
    ...
    if condition.ranges:
        for rng in condition.ranges:
            ...
            if rng.min is not None:
                range_filter.append(models.NodeFloatAttributeValue.float_value >= rng.min)
            if rng.max is not None:
                range_filter.append(models.NodeFloatAttributeValue.float_value <= rng.max)

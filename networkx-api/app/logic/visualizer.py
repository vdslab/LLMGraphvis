...
def _validate_config_keys(config: Dict, allowed_keys: Set[str], config_name: str):
    ...
    if unknown:
        raise ValueError(
            f"Invalid keys in {config_name}: {unknown}. Allowed: {allowed_keys}"
        )
...
def _validate_node_color_config(config: Optional[Dict]):
    ...
    if (
        "attribute" not in config
        and "scale_type" in config
        and config["scale_type"] != "STATIC"
    ):
    ...
    if "scale_type" in config:
        if config["scale_type"] not in ["LINEAR", "CATEGORICAL", "RANKING"]:
            raise ValueError(
                f"Invalid scale_type: {config['scale_type']}. Must be LINEAR, CATEGORICAL, or RANKING."
            )

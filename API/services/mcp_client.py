...
class NetworkXMCPClient:
    ...
    async def _call_tool(self, tool_name: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(
                f"Calling MCP tool: {tool_name} with parameters: {parameters}")
    ...
    async def calculate_and_store_centrality(
        ...
        if result.get("success"):
            logger.info(
                f"✅ Stage 1 completed: {centrality_type} centrality calculated and stored")
            if "result" in result:
    ...
    async def get_centrality_visualization(
        ...
        if result.get("success"):
            logger.info(
                f"✅ Stage 2 completed: Visualization data generated for {calculation_id}")
            if "result" in result:
                return result["result"]
    ...
    async def calculate_centrality_two_stage(
        ...
        """
        logger.info(
            f"🎯 Starting two-stage centrality calculation: {centrality_type}")
        ...
        # 両方のステージが成功
        logger.info(
            f"✅ Two-stage centrality calculation completed for {centrality_type}")
        return {
            "success": True,
            "centrality_type": centrality_type,
            "calculation_id": calculation_id,
